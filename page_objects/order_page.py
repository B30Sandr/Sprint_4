...
class OrderPage(MainPage):
    ...
    def input_first_name(self, first_name: str):
        return self.find_element(OrderPageLocator.FIRST_NAME_INPUT).send_keys(first_name)
    ...
    def input_telephone_number(self, telephone_number: str):
        return self.find_element(OrderPageLocator.TELEPHONE_NUMBER_FIELD).send_keys(telephone_number)
    ...
    def input_date(self, date: str):
        return self.find_element(OrderPageLocator.DATE_FIELD).send_keys(date)

    @allure.step('Выбор периода аренды')
    def choose_rental_period(self, option: int):
        self.find_element(OrderPageLocator.RENTAL_PERIOD_FIELD).click()
        return self.find_elements(OrderPageLocator.RENTAL_PERIOD_LIST)[option].click()

    @allure.step('Выбор цвета')
    def choose_color(self, option: int):
        return self.find_elements(OrderPageLocator.COLOR_CHECKBOXES)[option].click()

    @allure.step('Комментарий для курьера')
    def input_comment(self, comment_text):
        return self.find_element(OrderPageLocator.COMMENT_FOR_COURIER_FIELD).send_keys(comment_text)
    ...
    def get_order_number(self):
        about_order_text = self.find_element(OrderPageLocator.ORDER_COMPLETED_INFO).text
